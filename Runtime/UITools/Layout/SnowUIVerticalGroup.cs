...
namespace Snowdrama.UI
{
    ...
    /// </summary>
    [ExecuteInEditMode]
    public class SnowUIVerticalGroup : MonoBehaviour
    {
        [Header("Gap")]
        ...
        void LateUpdate()
        {
            if (transform.childCount != children.Count || forceUpdate)
            {
                ...
                if (children.Count > 0)
                {
                    ...
                    {
                        children[i].anchorMin = new Vector2(0 + (gapX / 2), 1 - ((percentWidth * (i + 1)) - (gapY / 2)));
                        children[i].anchorMax = new Vector2(1 - (gapX / 2), 1 - ((percentWidth * (i + 0)) + (gapY / 2)));
